...
fun Søkerinformasjon.tilAvsnitt(språk: Språkkode) =
    Avsnitt(
...
private fun tittelAvsnitt(språk: Språkkode) =
    when (språk) {
        Språkkode.NB -> "Søker"
        else -> error("Mangler mapping av $språk")
    }

private fun labelFødselsnummer(språk: Språkkode) =
    when (språk) {
        Språkkode.NB -> "Fødselsnummer"
        else -> error("Mangler mapping av $språk")
    }

private fun labelNavn(språk: Språkkode) =
    when (språk) {
        Språkkode.NB -> "Navn"
        else -> error("Mangler mapping av $språk")
    }
